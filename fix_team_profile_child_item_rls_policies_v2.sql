-- V2: corrige el 403 al guardar perfiles de equipo cuando digital_cards.org_id
-- esta vacio pero el usuario dueno de la tarjeta si pertenece a la misma org.
--
...
AS $$
  SELECT u.id, u.org_id, lower(u.role) AS role
  FROM public.users u
  WHERE u.id = auth.uid()
    OR (
...
AS $$
  SELECT EXISTS (
    ...
    LEFT JOIN public.users card_owner ON card_owner.id = dc.user_id
    CROSS JOIN public.current_taploop_user() cu
    WHERE dc.id = p_card_id
      AND (
        dc.user_id = cu.id
        OR (
          cu.role IN ('admin', 'owner', 'super_admin')
          AND cu.org_id IS NOT NULL
          AND (
            dc.org_id = cu.org_id
            OR card_owner.org_id = cu.org_id
          )
        )
      )
  );
...
GRANT SELECT ON public.digital_cards TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.contact_items TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.social_links TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.smart_forms TO authenticated;
